...
Notes land in `00-Inbox/` (via `vim` or `/vagus-create-note`). PARA wants them filed into
Projects/Areas/Resources/Archive. How much should the tool automate that?

## Options considered

1. **Assisted, on demand** — `/vagus-process-inbox`: Claude reads each inbox note, proposes a destination +
...
- The `vagus-process-inbox` skill is **`disable-model-invocation: true`** (side-effecting; user-triggered) so
  Claude never silently reorganizes the vault. Generic “organize my notes” wording may prompt an
  offer to invoke it, never automatic invocation; each proposed move requires confirmation.
